...
void unpack_share(const std::string &buf, std::string &id_out, std::vector<int> &v_out) {
    ...
    uint32_t id_len = read_u32(p + off); off += 4;
    if (off + id_len + 4 > n) throw std::runtime_error("bad id_len");
    id_out.assign((const char*)p + off, id_len); off += id_len;

    uint32_t v_len = read_u32(p + off); off += 4;
    if (off + 4ull * v_len > n) throw std::runtime_error("bad vec_len");
